...
namespace tbt
{
	...
	bool RadixSort::run(cl::CommandQueue queue, cl_uint *a, cl_uint n)
	{
		...
		try {
			...
			m_buffer_a      = cl::Buffer(getContext(), CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, n*sizeof(cl_uint), a);
			m_buffer_b      = cl::Buffer(getContext(), CL_MEM_READ_WRITE, n*sizeof(cl_uint));
			m_buffer_gcount = cl::Buffer(getContext(), CL_MEM_READ_WRITE, m_numGroups*BASE*sizeof(cl_uint));

			if(m_numPrescanGroups > 256)
				m_buffer_psum = cl::Buffer(getContext(), CL_MEM_READ_WRITE, 256*sizeof(cl_uint));
